...
def parse_score(review, category):
    try:
        sp = []
        if category != "math":
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.output_review_file):
        review_jsons = get_json_list(args.output_review_file)
        assert len(review_jsons) == len(question_jsons)
        reviews = [_["text"] for _ in review_jsons]
        review_update = True
    ...
    for i in question_idx_list:
        ...
        if not review_update:
            ...
            reviews.append(get_eval(sys_prompt, prompt, 32 if cat != "math" else 2048))
        else:
            assert review_jsons[i]["question_id"] == question_jsons[i]["question_id"]
            if review_jsons[i]["score"][0] >= 0 and review_jsons[i]["score"][1] >= 0:
                continue
            reviews[i] = get_eval(sys_prompt, prompt, 2048)
    ...
    with open(f"{args.output_review_file}", "w") as output_review_file:
        for idx, review in enumerate(reviews):
            scores = parse_score(review, question_jsons[idx]["category"])
