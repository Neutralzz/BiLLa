...
import task_instruct_tuning_args as args
...
class DataProcessor(threading.Thread):
    def __init__(self, data_root, data_files, tokenizer, batch_size, max_seq_length):
        ...
        if 'translate' in data_files:
            self.translate_prompts = {"en-zh":[], "zh-en":[]}
            for k in self.translate_prompts:
                with open(os.path.join(data_root, f'translate/{k}.prompt'), 'r', encoding='utf-8') as f:
                    self.translate_prompts[k] = [line.strip() for line in f]
        
        if os.path.exists(f"{data_root}/prompts"):
            self.other_prompts = {}
            for filename in os.listdir(f"{data_root}/prompts"):
                if not filename.endswith("prompt"):
                    continue
                key = filename.replace('.prompt', '')
                with open(f"{data_root}/prompts/{filename}", "r", encoding="utf-8") as f:
                    self.other_prompts[key] = [line.strip() for line in f]
    ...
    def make_features(self, examples):
        ...
        for example in examples:
            if "output" not in example:
                input_ids = self.tokenizer.encode(example["input"])[:self.max_seq_length]
                pos = random.randint(int(0.2 * len(input_ids)) + 1, int(0.7 * len(input_ids)))
                loss_mask = [0] * pos + [1] * (len(input_ids) - pos)
            else:
                if "pre_prompt" in example:
                    prefix = example["pre_prompt"] + "\n" + example["input"].rstrip() + "\n\n"
                elif "post_prompt" in example:
                    prefix = example["input"].rstrip() + "\n" + example["post_prompt"] + "\n"
                else:
    ...
    def run(self):
        ...
        while self.running and tid < len(tags):
            tag = tags[tid]
            if tag == 'translate':
                t_sample = self.get_sample(tag)
                en_text = t_sample["en_text"]
                zh_text = t_sample["zh_text"]
                pattern = random.choice(["en-zh", "zh-en"])
                prompt = random.choice(self.translate_prompts[pattern])
                if pattern == "en-zh":
                    batch.append({"input": en_text, "post_prompt": prompt, "output": zh_text})
                else:
                    batch.append({"input": zh_text, "post_prompt": prompt, "output": en_text})
            elif tag in ['zh', 'en']:
                batch.append({"input": self.get_sample(tag)["text"]})
            else:
                t_sample = self.get_sample(tag)
                input_text = t_sample["input"]
                output_text = t_sample["output"].rstrip()
                prompt_type = t_sample["type"]
                prompt_pos = random.choice(["post", "pre"])
                prompt_key = f"{prompt_type}.{prompt_pos}"
                if len(prompt_type) == 0 or prompt_key not in self.other_prompts:
                    batch.append({"input": input_text, "output": output_text})
                else:
                    prompt = random.choice(self.other_prompts[prompt_key]).rstrip()
                    batch.append({"input": input_text, "output": output_text, f"{prompt_pos}_prompt": prompt})
