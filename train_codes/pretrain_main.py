    ...
    def make_features(self, examples):
        ...
        for example in examples:
            if example['prompt'] is None:
                ...
                input_ids_1 = self.tokenizer.encode(example['input']+'\n'+example['prompt'], True, False)
    ...
    def run(self):
        ...
        while self.running and tid < len(tags):
            ...
            if tag == 'translate':
                ...
                if pattern == "en-zh":
                    batch.append({"input": en_text, "prompt": prompt, "output": zh_text})
                else:
                    batch.append({"input": zh_text, "prompt": prompt, "output": en_text})
            else:
                batch.append({"input": self.get_sample(tag)["text"], "prompt": None, "output": None})
...
def get_data_files():
    ...
    data_files = {k: {'weight': v, 'files': []} for k, v in args.pretrain_data_split.items()}
